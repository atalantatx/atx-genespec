...
sel_target = "SCN9A"
n_trials = 50
output_path = "../outputs/RandomForestRegressor/primary_tiling/"
...
train = pd.DataFrame(gene_data.loc[gene_data["Screen"] == "Primary"])
...
model_filename = f"{model_path}{sel_target}_pt_xgbr_trail_{n_trials}_featset1_orig.pkl"
...
trend_plot(
    predictions,
    y_test,
    title=study.study_name + " " + str(n_trials) + " trials_test" + f"random state ({random_state})",
    outfile=output_path
    + "scatter_test_"
    + study.study_name
    + "_trials_"
...
trend_plot(
    ...
    + " "
    + str(n_trials)
    + f" trials_training" + f"random state ({random_state})",
    outfile=output_path
    + "scatter_training_"
    + study.study_name
    + "_trials_"
...
trend_plot(
    optimize_preds,
    y_opt,
    title=study.study_name + " " + str(n_trials) + " trials_optimizing" + f"random state ({random_state})",
    outfile=output_path
    + "scatter_optimize_"
    + study.study_name
    + "_trials_"
