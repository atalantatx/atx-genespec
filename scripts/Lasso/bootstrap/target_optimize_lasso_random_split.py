...
for i in range(doc["boostrap_reps"]):
    ...
    def objective(trial):
        params = {
            ...
            "n_alphas":trial.suggest_int("n_alphas",1,10),
            "eps":trial.suggest_float("eps",0.0001,2.0)
        }

        model = LassoCV(**params,n_jobs=16,max_iter=10000)
        ...
        print('R squared training set', round(model.score(X_train, y_train_scaled)*100, 2))
        print('R squared test set', round(model.score(X_opt, y_opt_scaled)*100, 2))
        # print(model.score(y_pred,y_opt_scaled))

        # r2score = r2_score(y_opt, y_pred)
        # r2_score_val = r2_score(y_opt, y_pred)ß
    ...
    print(best_params)
    model = LassoCV(**best_params, n_jobs=16)
