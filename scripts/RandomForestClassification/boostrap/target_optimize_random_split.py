...
for i in range(doc["boostrap_reps"]):
    ...
    # %%
    X_train        = train.loc[train["Active"]!=999,available_features]
    y_train        = train.loc[train["Active"]!=999,"Active"]
    y_train_scaled = train.loc[train["Active"]!=999,"Scaled"]
    ...
    def objective(trial):
        ...
        model = RandomForestClassifier(**params, n_jobs=32, random_state=7,bootstrap=True,n_estimators=doc["n_estimators"])
        ...
        # mae = mean_absolute_error(y_opt, y_pred[:,0])
        ...
        CM = confusion_matrix(y_opt, y_pred)
        FN = CM[1][0]
        TP = CM[1][1]
        ...
        res = pearsonr(y_opt_scaled, y_pred_prob[:,0]) #Unable to use statistic with xgb
    ...
    training_preds = model.predict_proba(X_train)[:,0]
    optimize_preds = model.predict_proba(X_opt)[:,0]
